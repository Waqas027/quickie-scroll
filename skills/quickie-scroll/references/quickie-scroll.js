...
function mountQuickieScroll(container, config) {
  ...
  function renderAct(a) {
    ...
    if (a.kind === 'statement') {
      ...
      s.innerHTML =
        `<span class="sw-act__brand">${esc(a.brand || '')}</span>` +
        `<nav class="sw-act__links">${(a.links || []).map(l =>
          `<a href="${esc(l.href || '#')}">${esc(l.label || '')}</a>`).join('')}</nav>` +
...
function injectCSS() {
  ...
  .sw-act--footer{display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:18px;padding-block:clamp(40px,7vh,72px);font-size:.86rem;}
  ...
  .sw-act__links a{color:inherit;text-decoration:none;opacity:.7;} .sw-act__links a:hover{opacity:1;}
